...
def getFitness(deck, population):
    numOfGames = 3
    wins = 0
    all_decks = copy.deepcopy(population)

    removeDeck(all_decks, deck)

    opponents = random.sample(range(len(all_decks)), numOfGames)  # list of opponent by index
    results = []
...
def crossoverPopulation(population):
    ...
    for i in range(numOfRecombinations):
        ind1, ind2 = tournamentSelection(tempPop,
                                         3)  # tournament size of 3, hardcoded (can be dynamically allocated later)
        child1, child2 = recombination(ind1, ind2)
        tempPop.append(child1)
        tempPop.append(child2)

    calculateFitness(tempPop)  # get new fitness for the new members of the population

    tempPop.sort(key=attrgetter("fitness"), reverse=True)  #
